...
type value = VInt of int | VBool of bool | VPair of value * value | VUnit
...
let print_value (v : value) : unit = v |> show_value |> print_string
...
let rec eval (env : env) (e : expr) : value =
  ...
  | Match (p, x, y, e) -> (
      ...
      | VPair (a, b) -> eval Env.(env |> add y b |> add x a) e
          ...
          | VInt m -> (
              let rec sm i acc =
                if i = m + 1 then Some (VInt acc)
                  ...
                  match eval (Env.add x (VInt i) env) k with
              ...
              | None ->
                  failwith
                    "SUM expression was evaluated to a non integer value!")
